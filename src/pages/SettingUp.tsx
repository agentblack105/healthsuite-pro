...
const steps = [
  ...
  { icon: Layers, label: "Arranging your modules", sub: "OPD, IPD, Billing, Lab…" },
...
const SettingUp = () => {
  ...
  const setupData = location.state as Record<string, unknown> | null;
    ...
    const interval = setInterval(() => {
      setActive((prev) => {
        if (prev >= steps.length - 1) {
          ...
          setTimeout(() => navigate("/dashboard", { state: setupData }), 2000);
          return prev;
        }
        return prev + 1;
      });
